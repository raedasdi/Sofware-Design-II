 ...
 * also will only move if it has enough energy and
 * if there are no critters in the future spot.
...
public class Critter1 extends TestCritter {
	...
	public void doTimeStep() {
		...
		if(getEnergy() > Params.RUN_ENERGY_COST && critterLook(dir, true) == null) {
			run(dir);
		} else if(getEnergy() > Params.WALK_ENERGY_COST && critterLook(dir, false) == null) {
			walk(dir);
		}
	...
	public static String runStats(List<Critter> critter1s) {
		...
		String output1 = (critter1s.size() + " total Critter1s    " + reproduceAble / (0.01 * critter1s.size()) + "% able to repproduce   " + endangered/ (0.01 * critter1s.size()) + "% in danger of dying   ");
